...
static void dsp_handleEcho(Dsp* dsp, int* outputL, int* outputR) {
  ...
  dsp->firBufferL[dsp->firBufferIndex] = (
    dsp->apu->ram[adr] + (dsp->apu->ram[(adr + 1) & 0xffff] << 8)
  );
  dsp->firBufferL[dsp->firBufferIndex] >>= 1;
  dsp->firBufferR[dsp->firBufferIndex] = (
    dsp->apu->ram[(adr + 2) & 0xffff] + (dsp->apu->ram[(adr + 3) & 0xffff] << 8)
  );
  ...
  *outputR = outR < -0x8000 ? -0x8000 : (outR > 0x7fff ? 0x7fff : outR); // clamp 16-bit
  ...
  inR &= 0xfffe;
  if(dsp->echoWrites) {
    dsp->apu->ram[adr] = inL & 0xff;
    dsp->apu->ram[(adr + 1) & 0xffff] = inL >> 8;
    dsp->apu->ram[(adr + 2) & 0xffff] = inR & 0xff;
    dsp->apu->ram[(adr + 3) & 0xffff] = inR >> 8;
  }
  // handle indexes
  dsp->firBufferIndex++;
...
static void dsp_cycleChannel(Dsp* dsp, int ch) {
  ...
  if(dsp->evenCycle) {
    ...
    if(dsp->channel[ch].keyOff) {
      ...
      dsp->channel[ch].decodeOffset = dsp->apu->ram[samplePointer];
      dsp->channel[ch].decodeOffset |= dsp->apu->ram[(samplePointer + 1) & 0xffff] << 8;
...
static void dsp_decodeBrr(Dsp* dsp, int ch) {
  ...
  if(dsp->channel[ch].previousFlags == 1 || dsp->channel[ch].previousFlags == 3) {
    // loop sample
    uint16_t samplePointer = dsp->dirPage + 4 * dsp->channel[ch].srcn;
    dsp->channel[ch].decodeOffset = dsp->apu->ram[(samplePointer + 2) & 0xffff];
    dsp->channel[ch].decodeOffset |= (dsp->apu->ram[(samplePointer + 3) & 0xffff]) << 8;
  ...
  uint8_t header = dsp->apu->ram[dsp->channel[ch].decodeOffset++];
  int shift = header >> 4;
